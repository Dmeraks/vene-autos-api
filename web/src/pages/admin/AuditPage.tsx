...
const btnDetail =
  'min-h-[44px] w-full rounded-xl border border-brand-200 bg-white px-3 py-2.5 text-sm font-semibold text-brand-800 hover:bg-brand-50 dark:border-brand-800 dark:bg-slate-900 dark:text-brand-200 dark:hover:bg-brand-950/40 sm:min-h-0 sm:w-auto'
...
function AuditRowCard({ row, onOpenDetail }: { row: AuditItem; onOpenDetail: (row: AuditItem) => void }) {
  ...
  return (
    <article className="va-card !p-4" aria-label={`Evento ${actionLabel}`}>
      ...
      <p className="mt-2 text-sm text-slate-700 dark:text-slate-300">
        <span className="text-slate-500 dark:text-slate-400">Quién: </span>
...
function AuditDetailModal({ row, onClose }: { row: AuditItem; onClose: () => void }) {
  ...
  return (
    <div
      className="fixed inset-0 z-[90] flex items-center justify-center bg-slate-900/55 p-4 dark:bg-black/65"
      role="presentation"
    >
      <div
        className="va-card max-h-[min(90dvh,920px)] w-full max-w-lg overflow-y-auto shadow-2xl"
        role="dialog"
        aria-modal="true"
      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <h2 id="audit-detail-title" className="mt-1 text-lg font-semibold text-slate-900 dark:text-slate-50">
              {title}
            </h2>
          ...
          <button
            type="button"
            onClick={onClose}
            className="shrink-0 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-700"
          >
            Cancelar
          </button>
        </div>

        <dl className="mt-4 grid gap-2 rounded-xl border border-slate-100 bg-slate-50/80 px-3 py-3 text-sm dark:border-slate-800 dark:bg-slate-800/40">
          <div>
            <dt className="text-xs font-medium uppercase tracking-wide text-slate-500 dark:text-slate-400">Cuándo</dt>
            <dd className="font-mono text-xs text-slate-800 dark:text-slate-200">{when}</dd>
          </div>
          <div>
            <dt className="text-xs font-medium uppercase tracking-wide text-slate-500 dark:text-slate-400">Quién</dt>
        ...
        <div className="mt-5 space-y-5">
          {sections.map((s) => (
            <section key={s.heading}>
              <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-50">{s.heading}</h3>
        ...
        <details className="mt-6 rounded-xl border border-slate-200 bg-slate-50/50 dark:border-slate-700 dark:bg-slate-800/30">
    ...
    <div className="space-y-5 sm:space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">
          Historial de acciones relevantes (caja, usuarios, órdenes, etc.). Usá <strong className="font-semibold">Ver detalles</strong> en
          cada fila para leer el contexto en español y el JSON completo si lo necesitás.
        </p>
      </div>
      <form
        className="va-card space-y-3 sm:grid sm:grid-cols-12 sm:gap-3 sm:space-y-0"
        onSubmit={(e) => {
          e.preventDefault()
          setPage(1)
          void search(1)
      ...
      >
        <p className="text-xs text-slate-500 dark:text-slate-400 sm:col-span-12">
        ...
        <div className="flex items-end sm:col-span-2">
          <button
            type="submit"
            className="min-h-[44px] w-full rounded-xl bg-brand-600 py-2.5 text-sm font-semibold text-white hover:bg-brand-700 sm:min-h-0"
          >
            Buscar
          </button>
        </div>
        ...
        <>
            ...
            <table className="w-full min-w-[780px] text-left text-sm">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/90 text-xs uppercase tracking-wide text-slate-500 dark:border-slate-700 dark:bg-slate-800/80 dark:text-slate-400">
                  ...
                  <th className="px-3 py-3">Qué ocurrió</th>
                  <th className="px-3 py-3">Sobre</th>
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className="min-h-[44px] rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-800 hover:bg-slate-50 disabled:opacity-40 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 dark:hover:bg-slate-700"
            >
              Anterior
            </button>
            <button
