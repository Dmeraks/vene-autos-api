...
import { WORK_ORDER_CHANGED_EVENT } from '../lib/workOrderEvents'

/**
 * Solo estados que cuentan como «pendiente» hasta que la OT pasa a entregada
 * (entregadas y canceladas no son notificación: dejan de sumar automáticamente).
 */
const ALERT_PIPELINE_STATUSES = [
  'UNASSIGNED',
  'RECEIVED',
  'IN_WORKSHOP',
  'WAITING_PARTS',
...
type AlertPipelineStatus = (typeof ALERT_PIPELINE_STATUSES)[number]

const WO_STATUS_TITLE_ES: Record<AlertPipelineStatus, string> = {
...
const WO_ALERT_ROWS: readonly { status: AlertPipelineStatus; description: string }[] = [
...
async function fetchStatusTotal(status: WorkOrderStatus): Promise<number> {
    ...
    try {
      const results = await Promise.all(
        WO_ALERT_ROWS.map(async ({ status }) => {
          const total = await fetchStatusTotal(status)
